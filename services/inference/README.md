...
Proto at `protos/ai_service.proto`:
...
message PredictRequest { string text = 1; string model_id = 2; } // model_id: spark|flare default spark, truncated 64
message AnalyzeDocumentEvent { oneof event { AnalyzeDocumentStarted started=1; AnalyzeDocumentProgress progress=2; PredictResponse final=3; } }
message HighlightSpan { int32 char_start=1; int32 char_end=2; float ai_confidence=3; } // 0..100
message PredictResponse { string model_name=1; string label=2; bool is_ai_generated=3; float confidence_score=4; float human_confidence=5; float ai_confidence=6; repeated HighlightSpan highlight_spans=7; }
```

```text
Health  grpc.health.v1.Health/Check|Watch -> SERVING/NOT_SERVING
Metrics GET :8333/metrics -> Prometheus
Detect  -> OK PredictResponse | INVALID_ARGUMENT (unsupported model, empty chunks, bad offsets) | RESOURCE_EXHAUSTED (queue full/worker unavailable/circuit open) | CANCELLED | INTERNAL
AnalyzeDocument stream -> first started{total_chars,total_chunks} | progress{processed_chunks,total_chunks} monotonic | final PredictResponse | RESOURCE_EXHAUSTED/INVALID_ARGUMENT/INTERNAL
Auth    -> UNAUTHENTICATED (missing/invalid/expired Bearer or bad x-api-key)
```

* `servicers.py:18` truncates `model_id` at `64` then `lower().strip()`, defaults to `spark`; logs truncated at `500` chars.
* `_MAX_TEXT_LOG_LEN=500` prevents log injection; `_MAX_TOKEN_LEN=8192` guards DoS.
