...
class TokenChunker(Protocol):
    def chunk(self, text: str, chunk_size: int, stride: int) -> list[DocumentChunk]:
        ...
...
class ChunkPlanner:
    def __init__(self, chunker: TokenChunker, chunk_size: int, stride: int):
        self.chunker = chunker
        self.chunk_size = chunk_size
        self.stride = stride

    def plan(self, text: str) -> list[DocumentChunk]:
        chunks = self.chunker.chunk(text, self.chunk_size, self.stride)
        if chunks:
            return chunks
...
def build_chunk_planner(tokenizer, chunk_size: int, stride: int) -> ChunkPlanner:
    if callable(tokenizer):
        return ChunkPlanner(BertTokenChunker(tokenizer), chunk_size, stride)
    return ChunkPlanner(RegexTokenChunker(), chunk_size, stride)
