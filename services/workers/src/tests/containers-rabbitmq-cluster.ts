...
/**
 * 3-node RabbitMQ cluster for accuracy-over-speed HA tests. No Compose:
 ...
 * election, single-node survival). Floci stays Terraform-only.
 */
    ...
    for (let i = 1; i < 3; i++) {
      ...
      for (let attempt = 0; attempt < 12 && !joined; attempt++) {
        try {
          ...
          lastErr = e;
          // Ensure app is running before next attempt.
          try {
