...
def test_extract_non_multipart_body_returns_422(client, mocker):
    ...
    mocker.patch(
        "app.api.v1.endpoints.extract.run_extraction_task",
        return_value=MagicMock(),
    )

    response = client.post("/api/v1/extract", json={"file": "not-multipart"})

    assert response.status_code == 422


def test_extract_filename_none_falls_back_to_unknown(client, mocker):
    from app.domain.entities import ExtractionResult as NewResult

    mocker.patch("app.api.deps.magic.from_buffer", return_value="text/plain")
    mocker.patch(
        "app.api.v1.endpoints.extract.run_extraction_task",
        return_value=NewResult(text="body"),
    )
