...
def record_request(method: str, route: str, status_code: int, duration: float) -> None:
    ...
    HTTP_REQUESTS_TOTAL.labels(method=method, route=route, status_code=label).inc()
    HTTP_REQUEST_DURATION_SECONDS.labels(method=method, route=route, status_code=label).observe(duration)
    if status_code >= 400:
        HTTP_REQUEST_ERRORS_TOTAL.labels(method=method, route=route, status_code=label).inc()
