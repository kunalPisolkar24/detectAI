...
```yaml
healthcheck:
  # NOTE: plain GET fetch, not `wget --spider`: spider mode sends HEAD,
  # which gin rejects with 404 on the GET-only /readyz route.
  test: ["CMD", "wget", "--no-verbose", "--tries=1", "-O", "/dev/null", "http://localhost:8080/readyz"]
  interval: 10s
  timeout: 5s
  retries: 3
  start_period: 15s
