...
|---|---|---|
| `chat-mongo` atom | `mongosh --eval db.adminCommand('ping')` | `10s/5s/10r/10s start` |
| `chat-redis` atom | `redis-cli -a $PASS ping` | `5s/3s/10r` |
...
Compose `depends_on: chat-mongo/chat-redis service_healthy` gates `chat-service/worker`; load `k6 depends_on chat-service service_healthy` (`infra/compose.load.yml:99`).

## Ports

* `chat-mongo 27018:27017`, `chat-redis 6381:6379` in `infra/compose.yml:10` (isolated from generic `27017/6379`); load internal-only (no host ports, `chat_loadnet`).
