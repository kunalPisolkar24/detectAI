...
| `CHAT_REDIS_ADDR` | *(required)* | `host:port` | single primary — `redis-chat:6379` in compose, ElastiCache primary `host:port` (Floci `localhost:6380`, AWS DNS). Backed by `*redis.Client` (no cluster). |
| `REDIS_CHAT_PASSWORD` | *(empty / `test_redis_password` load)* | `string` | `*redis.Client` + `redis-cli -a` healthcheck; redis-chat atom defaults for local; ElastiCache `auth_token` via `REDIS_PASSWORD` |
...
```go
// config.go:40
require MONGO_URI, CHAT_REDIS_ADDR non-empty (CHAT_REDIS_ADDR must be host:port)
