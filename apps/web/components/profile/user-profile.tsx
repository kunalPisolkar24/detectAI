...
export const UserProfile: React.FC = () => {
 ...
 const handleSave = async () => {
    ...
    try {
        ...
        setIsEditing(false);
        await updateSession({ name: updatedData.name }); // Update session name
  ...
  const handleConfirmCancelSubscription = async () => {
      ...
      toast.info("Scheduling subscription cancellation...");
      try {
          ...
          if (!response.ok) {
              const errorData = await response.json();
              throw new Error(errorData.error || `Failed to schedule cancellation: ${response.statusText}`);
          }
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl py-20 text-center">
          ...
          <p className={cn(theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600')}>
           ...
           {sessionStatus === 'authenticated' && error && (
                <Button onClick={fetchProfile} className="mt-6">Retry</Button>
           )}
        </div>
      </ScrollArea>
    );
  ...
  const displayIsCancellationScheduled = user.isCancellationScheduled;
                  ...
                  <>
                    ...
                    {displayIsCancellationScheduled ? (
                      <div className="text-center text-xs sm:text-sm p-3 rounded-md bg-orange-500/10 text-orange-600 dark:bg-orange-600/15 dark:text-orange-400 border border-orange-500/30 flex items-center justify-center gap-2">
                          <Info size={14} />
                          <span>Access ends {displayPremiumExpiry ? format(new Date(displayPremiumExpiry), "MMMM d, yyyy") : 'at period end'}.</span>
                      </div>
                    ) : (
                      <>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent className={theme === 'dark' ? 'bg-zinc-900 border-zinc-700' : 'bg-white'}>
                            <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                    Your premium benefits will remain active until the end of the current period ({displayPremiumExpiry ? format(new Date(displayPremiumExpiry), "MMM d, yyyy") : 'date unknown'}). This will schedule the cancellation.
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            <AlertDialogFooter>
                                <AlertDialogCancel disabled={isCancelling}>Keep Subscription</AlertDialogCancel>
                                <AlertDialogAction onClick={handleConfirmCancelSubscription} disabled={isCancelling} className="bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-800 text-white disabled:opacity-50 disabled:cursor-not-allowed">
                                    {isCancelling ? <Loader2 size={14} className="mr-2 animate-spin"/> : null} Yes, Schedule Cancellation
                                </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
                  ...
                  <>
                    ...
                    <Dialog open={showPricingDialog} onOpenChange={setShowPricingDialog}>
                        ...
                        <Button size="sm" className={cn("w-full font-semibold", theme === "dark" ? "bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white shadow-md hover:shadow-lg hover:shadow-blue-500/20" : "bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white shadow-md hover:shadow-lg hover:shadow-blue-500/30")}>
                            <Sparkles size={14} className="mr-2" /> Re-subscribe to Premium
                        </Button>
                        </DialogTrigger>
                  ...
                  <div>
                    ...
                    <div>
                      ...
                      {displaySubscriptionStatus === "ACTIVE" && displayIsCancellationScheduled ? (
                          ...
                          <Badge variant="default" className={cn("px-2 py-0.5 text-xs font-medium border-none capitalize",
                             displaySubscriptionStatus === "ACTIVE" || displaySubscriptionStatus === "TRIALING" ? "bg-green-500/90 dark:bg-green-600/90 text-white" :
                             displaySubscriptionStatus === "CANCELED" ? "bg-yellow-500/90 dark:bg-yellow-600/90 text-black" :
                             displaySubscriptionStatus === "PAST_DUE" || displaySubscriptionStatus === "PAUSED" ? "bg-red-500/90 dark:bg-red-600/90 text-white" :
                             "bg-gray-500/90 dark:bg-gray-600/90 text-white"
                          )}>
                              {displaySubscriptionStatus ? displaySubscriptionStatus.toLowerCase() : "Free"}
                           </Badge>
                      )}
                    </div>
