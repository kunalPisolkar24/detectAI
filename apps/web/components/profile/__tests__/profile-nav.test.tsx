...
describe('ProfileNav Component', () => {
  ...
  it('should render initials "U" if user name is not present', () => {
    mockUseSession.mockReturnValue({
      data: { user: { email: 'test@example.com' }, expires: 'never' },
  ...
  it('should call setTheme when theme toggle item is clicked (dark to light)', async () => {
    ...
    const themeToggleItem = await screen.findByText(/Light Mode/i);
    await act(async () => {
  ...
  it('should call setTheme when theme toggle item is clicked (light to dark)', async () => {
    ...
    const themeToggleItem = await screen.findByText(/Dark Mode/i);
  ...
  it('should link to the chat page', async () => {
    renderComponent();
    const avatarTrigger = screen.getByTestId('avatar');
     await act(async () => {
      fireEvent.click(avatarTrigger);
    ...
    expect(chatLink.querySelector('[data-testid="message-circle-icon"]')).toBeInTheDocument();
  });
});