...
interface PaymentProps {
  // Callback function to notify parent when subscription attempt starts
  onSubscriptionAttempt: () => void;
}

// Component accepts the callback prop
export const Payment: React.FC<PaymentProps> = ({ onSubscriptionAttempt }) => {
      ...
      initializePaddle({
        ...
        eventCallback: (data) => {
          ...
          if (data.name === 'checkout.completed') {
            ...
            // Optionally: You could call another prop here to indicate completion,
            // but webhook is the reliable source.
          }
        }
      })
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      setCurrentUser({ email: "test.user@example.com", id: "user_mock_123" });
  ...
  const products = [
    ...
    },
    {
      ...
      buttonAction: handleSubscription, // Action assigned here
    },
  ];

  if (!mounted) return null;

  const isLoading = isPaddleLoading || isLoadingUser;
                  ...
                  {isPremium ? (
                    <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                      <Button
                        ...
                        disabled={isLoading || !paddle || !currentUser}
                        size="sm"
                        className={cn(
                      ...
                      >
                        ...
                        {isLoading ? "Loading..." : product.buttonText}
                      </Button>
                    </motion.div>
                  ) : (
                    <Link href={product.buttonLink || '#'} passHref legacyBehavior>
                       <motion.a whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}>
                         <Button size="sm" className={cn("w-full font-semibold", theme === "dark" ? "bg-transparent border border-white/20 hover:bg-white/10 text-white" : "bg-transparent border border-black/20 hover:bg-black/5 text-black")} variant="outline">
                           {product.buttonText}
                         </Button>
                       </motion.a>
                    </Link>
