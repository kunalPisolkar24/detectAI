 ...
 *
 * Layout (all in ONE instance, `REDIS_URL` — standalone docker or ElastiCache users):
 ...
 * - `analytics:usage:event:{id}` → "1" EX 7d (usage dedup)
 *
 * Email is hashed (sha256 normalized, 16 hex chars) to avoid PII/special
 * chars in keys and to match the worker scheme.
 */
